...
- UI: light theme (#F7F5F0 bg, Fraunces headings, Instrument Sans body, IBM Plex Mono numbers). Wide layout 1360px; anchors = table + sticky right rail of explainer cards; backtest = 2-col grid of 4 question-framed tables. Radar: This week -> Pressure board (top-6 dov <=90d) -> Just passed (last 30d with ret5 chips, opens modal on History tab) -> Horizon slider 15-180d -> ledger. Company modal: stats grid, capital bar, timeline, anchor chips (x n serial tags), UPCOMING/HISTORY tabs (history = close-before + ret1/5/20 boxes + benchmark-vs-typical line from STATS), QTY/DATE math per upcoming event. Anchors page: sortable headers (click, gold arrows), search box, verdict pills, "only well-tested 10+" checkbox, fund-name click -> score breakdown overlay.
...
2. If sandbox copy missing: fetch raw.githubusercontent.com/TaxPracto/sme-lockin-tracker/main/build_site.py etc. via web_fetch (large files truncate ~60KB into tool-results files — grep them), or ask Ashwani to re-clone. Data files in sandbox are STALE STUBS — real data lives on the runner; never conclude "data missing" from local files (past bug: concluded GetFive missing when live registry had it).
3. After every edit: python3 -c "import ast; ast.parse(open('build_site.py').read())" then python3 build_site.py and probe docs/*.html with asserts.
4. Deploy via Chrome (no git in sandbox): navigate github.com/TaxPracto/sme-lockin-tracker/upload/main -> find file input -> file_upload with the WINDOWS path C:\Users\ASHWANI\...\outputs\sme-lockin-tracker\build_site.py -> form_input commit summary -> submit via javascript: [...document.querySelectorAll('button[type=submit]')].find(b=>b.textContent.trim()==='Commit changes') then btn.closest('form').requestSubmit(btn). Coordinate clicks are flaky; requestSubmit is not.
...
- _ANCH_JS is a plain """string""" appended after the anchors f-string, with .replace("__FD__", json.dumps(_FDATA)) at concat time. New anchors-page JS/CSS goes THERE (single braces OK).
- Share-count regex must match comma-grouped numbers WITHOUT "shares" suffix (raw HTML has <!-- --> between).
- Fund names normalize via .title() — causes "Hdfc Bank", "Ccv" cosmetics (known, accepted).
